...
proc.SetHasher(codec.HashArgon2, customArgon2)

// Configure custom masker
proc.SetMasker(codec.MaskEmail, customEmailMasker)
```
...
proc.SetEncryptor(codec.EncryptAES, aesEnc).
    SetEncryptor(codec.EncryptRSA, rsaEnc).
    SetHasher(codec.HashArgon2, argon2)
```
...
proc1, _ := codec.Use[User](json.New())

// Subsequent calls return cached instance
proc2, _ := codec.Use[User](json.New())
...
```go
jsonProc, _ := codec.Use[User](json.New())   // Cached as (User, "application/json")
yamlProc, _ := codec.Use[User](yaml.New())   // Cached as (User, "application/yaml")
```
...
```go
proc, _ := codec.Use[User](json.New())
proc.SetEncryptor(codec.EncryptAES, enc) // Configures the cached instance
```
...
func TestSomething(t *testing.T) {
    codec.Reset() // Start with empty registry
    defer codec.Reset() // Clean up after test

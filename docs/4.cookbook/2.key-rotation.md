...
Codec's `SetEncryptor` method allows runtime key changes, but doesn't track which key encrypted which data. This guide covers patterns for managing key rotation.
...
```go
// Original key
enc1, _ := codec.AES(key1)
proc.SetEncryptor(codec.EncryptAES, enc1)

stored, _ := proc.Store(ctx, &user) // Encrypted with key1

// Rotate key
enc2, _ := codec.AES(key2)
proc.SetEncryptor(codec.EncryptAES, enc2)
...
type versionedEncryptor struct {
    current    codec.Encryptor
    currentVer byte
    previous   map[byte]codec.Encryptor
}
...
func NewVersionedEncryptor(currentKey []byte, currentVer byte) *versionedEncryptor {
    enc, _ := codec.AES(currentKey)
    return &versionedEncryptor{
        current:    enc,
        currentVer: currentVer,
        previous:   make(map[byte]codec.Encryptor),
    }
}
...
proc.SetEncryptor(codec.EncryptAES, venc)
...
Use `codec.Envelope` for built-in key separation. The master key encrypts data keys, not data directly.

```go
masterKey := loadMasterKey() // From KMS, vault, etc.
enc, _ := codec.Envelope(masterKey)
...
func ReencryptEnvelope(oldMaster, newMaster []byte, ciphertext []byte) ([]byte, error) {
    oldEnc, _ := codec.Envelope(oldMaster)
    newEnc, _ := codec.Envelope(newMaster)
...
func (u *User) Decrypt(encryptors map[codec.EncryptAlgo]codec.Encryptor) error {
    ...
    legacyEnc := encryptors[codec.EncryptAlgo("aes-legacy")]
...
```go
proc.SetEncryptor(codec.EncryptAES, currentEnc)
proc.SetEncryptor(codec.EncryptAlgo("aes-legacy"), legacyEnc)
```

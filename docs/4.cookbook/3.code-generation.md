...
func (u *User) Mask(maskers map[codec.MaskType]codec.Masker) error {
    u.Email = maskers[codec.MaskEmail].Mask(u.Email)
    u.SSN = maskers[codec.MaskSSN].Mask(u.SSN)
    // Phone not masked!
    return nil
}
```
...
func (x *{{ $type }}) Encrypt(encryptors map[codec.EncryptAlgo]codec.Encryptor) error {
{{- range .Fields }}
{{- if .EncryptAlg }}
    if x.{{ .Name }} != "" {
        enc := encryptors[codec.EncryptAlgo("{{ .EncryptAlg }}")]
...
func (x *{{ $type }}) Decrypt(encryptors map[codec.EncryptAlgo]codec.Encryptor) error {
{{- range .Fields }}
{{- if .DecryptAlg }}
    if x.{{ .Name }} != "" {
        enc := encryptors[codec.EncryptAlgo("{{ .DecryptAlg }}")]
...
{{ if hasHash .Fields -}}
func (x *{{ $type }}) Hash(hashers map[codec.HashAlgo]codec.Hasher) error {
{{- range .Fields }}
{{- if .HashAlg }}
    if x.{{ .Name }} != "" {
        hasher := hashers[codec.HashAlgo("{{ .HashAlg }}")]
...
{{ if hasMask .Fields -}}
func (x *{{ $type }}) Mask(maskers map[codec.MaskType]codec.Masker) error {
{{- range .Fields }}
{{- if .MaskType }}
    if x.{{ .Name }} != "" {
        x.{{ .Name }} = maskers[codec.MaskType("{{ .MaskType }}")].Mask(x.{{ .Name }})
    }
{{- end }}
{{- end }}
    return nil
...
func (x *User) Encrypt(encryptors map[codec.EncryptAlgo]codec.Encryptor) error {
    if x.Email != "" {
        enc := encryptors[codec.EncryptAlgo("aes")]
...
func (x *User) Decrypt(encryptors map[codec.EncryptAlgo]codec.Encryptor) error {
    if x.Email != "" {
        enc := encryptors[codec.EncryptAlgo("aes")]
...
func (x *User) Hash(hashers map[codec.HashAlgo]codec.Hasher) error {
    if x.Password != "" {
        hasher := hashers[codec.HashAlgo("argon2")]
...
func (x *User) Mask(maskers map[codec.MaskType]codec.Masker) error {
    if x.Email != "" {
        x.Email = maskers[codec.MaskType("email")].Mask(x.Email)
    }
    if x.SSN != "" {
        x.SSN = maskers[codec.MaskType("ssn")].Mask(x.SSN)
    }
    return nil
}
